...
const EditMemorial = () => {
  ...
  const handleDeleteGalleryImage = async (imageId: string) => {
    const { error } = await supabase.from("memorial_images" as any).delete().eq("id", imageId);
    if (error) {
  ...
  const handleSubmit = async (isDraft: boolean) => {
    ...
    try {
      ...
      // Check photo limit before uploading new gallery images
      if (newGalleryImages.length > 0) {
        const limitCheck = await checkPhotoLimit(memorial.id, user.id);
        if (!limitCheck.allowed) {
          toast.error("You have reached the photo limit for your plan. Upgrade to add more images.");
